...
const PressReleaseDetail = () => {
  ...
  useEffect(() => {
    if (!pressRelease || pressRelease.images.length <= 1) return

    const interval = setInterval(() => {
      setCurrentImageIndex((prev) => (prev + 1) % pressRelease.images.length)
    }, 4000)
  ...
  const fetchPressRelease = async () => {
    ...
    try {
      const response = await axios.get<PressRelease>(`/api/press-releases/${id}`)
  ...
  const goToPrevious = () => {
    ...
    setCurrentImageIndex((prev) => (prev - 1 + pressRelease.images.length) % pressRelease.images.length)
  }
                ...
                <div className="mb-8 animate-in max-w-[730px] mx-auto" style={{ animationDelay: '0.2s' }}>
                  <div className="relative rounded-2xl overflow-hidden shadow-2xl bg-black h-[480px]">
                    ...
                    <img
                      ...
                      src={`/news-imgs/${pressRelease.images[currentImageIndex]}`}
                      alt={`${pressRelease.title} ${currentImageIndex + 1}`}
                    ...
                    {pressRelease.images.length > 1 && (
                      <div className="absolute top-4 right-4 px-3 py-1.5 rounded-full bg-black/50 backdrop-blur-sm text-white text-sm font-semibold">
                        {currentImageIndex + 1} / {pressRelease.images.length}
                      </div>
                    )}
                  </div>
                </div>
