...
const TemplateDocuments = () => {
  ...
  const fetchTemplateDocuments = async () => {
    try {
      ...
      const templateDocs = response.data.templates || []
      setDocuments(templateDocs)
      setError('')
    } catch (err) {
      console.error('Error fetching template documents:', err)
      setError('Failed to load template documents. Please try again later.')
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            <div className="inline-flex items-center px-4 py-2 rounded-full bg-secondary/20 backdrop-blur-sm border border-secondary/40 mb-6">
              ...
              <span className="text-sm font-semibold tracking-wider uppercase">Templates</span>
            </div>
            <h1 className="font-heading text-5xl md:text-6xl font-bold mb-6 leading-tight">
              Document Templates
            </h1>
            <p className="text-xl md:text-2xl text-gray-200 leading-relaxed">
              Access standardized templates and formats for developing and submitting climate projects
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {error && !loading && (
            <div className="bg-red-50 border border-red-200 text-red-600 px-6 py-8 rounded-xl font-body text-base text-center max-w-2xl mx-auto shadow-lg">
              ...
              <p>{error}</p>
              <button
                onClick={fetchTemplateDocuments}
              ...
              {filteredDocuments.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <h3 className="font-heading text-2xl font-bold text-primary mb-3">
                    {searchTerm ? 'No Matching Documents' : 'No Template Documents'}
                  </h3>
                  <p className="font-body text-lg text-text-secondary">
                    {searchTerm
                      ? 'Try adjusting your search terms'
                      : 'No template documents have been uploaded yet. Check back later!'}
                  </p>
                </div>
              ) : (
